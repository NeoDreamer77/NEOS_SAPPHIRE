# plugins/neos_timer/__init__.py
"""
NEOS Timer Plugin - Timer and reminder functionality with sound, desktop notifications, and phone alerts.
"""
...
def _load_config():
    """Load config from plugin config file."""
    global _config
    try:
        config_path = Path.home() / ".config/sapphire/plugins/neos_timer/config.json"
        if config_path.exists():
            import json
            with open(config_path) as f:
                _config.update(json.load(f))
    except Exception as e:
        logger.warning(f"Could not load timer config: {e}")


def _save_config():
    """Save config to plugin config file."""
    try:
        config_path = Path.home() / ".config/sapphire/plugins/neos_timer/config.json"
        ...
        logger.warning(f"Could not save timer config: {e}")
...
class Timer:
    ...
    def _trigger_alert(self):
        ...
        from plugins.neos_timer.sounds import play_sound
...
def get_status():
    """Get timer plugin status."""
    return {
        "active_timers": len(_timers),
        "timers": {name: {"remaining": timer.remaining_formatted(), "message": timer.message} 
                  for name, timer in _timers.items()},
        "config": _config
    }
