...
class VoiceChatSystem:
    def __init__(self):
        ...
        self._web_active_count = (
            0  # Ref-counted wakeword suppression during web UI activity
        )
        ...
        if fm.current_toolset_name and fm.current_toolset_name != "none":
            ...
            logger.info(
                f"Toolset '{fm.current_toolset_name}' re-applied after plugin scan"
            )
        ...
        try:
            ...
            try:
                ...
                if prompt_name and prompt_name != "__story__":
                    ...
                    logger.info(
                        "Startup prompt: story chat (prompt loaded from story engine)"
                    )
            ...
            if not prompt_name:
                chat_defaults_path = (
                    Path(__file__).parent / "user" / "settings" / "chat_defaults.json"
                )
            ...
            content = (
                ...
                else str(prompt_details)
            )
            self.llm_chat.set_system_prompt(content)
            prompts.set_active_preset_name(prompt_name)
            if (
                hasattr(prompts.prompt_manager, "scenario_presets")
                and prompt_name in prompts.prompt_manager.scenario_presets
            ):
            ...
            fallback_prompt = (
                "You are NEOS! You are the New Evolving Operating System. \n"
    ...
    def init_components(self):
        try:
            if config.WAKE_WORD_ENABLED:
                from core.wakeword.audio_recorder import (
                    AudioRecorder as RealAudioRecorder,
                )
                from core.wakeword.wake_detector import (
                    WakeWordDetector as RealWakeWordDetector,
                )

                self.wake_word_recorder = RealAudioRecorder()
                self.wake_detector = RealWakeWordDetector(
                ...
                from core.wakeword.wakeword_null import (
                    NullAudioRecorder,
        ...
        try:
            ...
            if provider_name == "sapphire_router":
                ...
                try:
                    ...
                    logger.warning(
                        f"No mic available — STT will work via web UI only: {mic_err}"
                    )
                    self.whisper_recorder = NullAudioRecorder()
    ...
    def _start_kokoro_server(self, base_dir):
        ...
        # Use neos-py311 Python which has kokoro installed
        py311_python = base_dir / "neos-py311" / "bin" / "python"
        self.tts_server_manager = ProcessManager(
            script_path=tts_script,
            log_name="kokoro",
            base_dir=base_dir,
            command_args=[str(py311_python), str(tts_script)],
        )
    ...
    def start_background_services(self):
        ...
        if (not provider or provider == "none") and getattr(
        ...
        stop_actions = [
            (
                ...
                lambda: (
                    ...
                    and self.agent_manager.shutdown()
                ),
            ),
            ...
            (
                ...
                lambda: (
                    hasattr(self, "continuity_scheduler")
                    and self.continuity_scheduler
                    and self.continuity_scheduler.stop()
                ),
            ),
            (
                "TTS server",
                lambda: self.tts_server_manager and self.tts_server_manager.stop(),
            ),
...
def run():
    ...
    print("Starting NEOS Voice Chat System")
    try:
    ...
    try:
        ...
        voice_chat.continuity_scheduler = (
            continuity_scheduler  # Attach for stop() and API routes
        )
        ...
        host_display = (
            "localhost"
            if config.WEB_UI_HOST in ("0.0.0.0", "127.0.0.1")
            else config.WEB_UI_HOST
        )
        ...
        BOLD = "\033[1m"
        RESET = "\033[0m"
        print(f"\n{CYAN_BG}{BLACK}{BOLD} ✨ NEOS IS NOW ACTIVE: {url} {RESET}\n")

        logger.info(f"NEOS is running. Starting uvicorn server...")
